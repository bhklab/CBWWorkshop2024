...
We can see that `gene_id` maps completely to the genes in our RNA-seq data,
whereas `hgnc_symbol` and `cosmic_gene_symbol` are missing gene symbols (see the
numbers under `FALSE`). This is a pretty obvious indicator to move forward with
the `gene_id` for downstream analysis.
...
provided annotation file to map back to the sample names in our model list.
...
the `SIDM02101` cell line model. We would represent such mutation events with `1`.
...
For the remainder of this session, we provide more sample datasets to practice
the above techniques.

```{r load_remaining_data}
