...
tre  |>
  aggregate(
      ...
      by=c("treatmentid", "sampleid", "bio_rep", "treatmentdose")
  ) |> 
  _[, .N, by=c("treatmentid", "sampleid", "bio_rep")] |>
  with(expr=hist(N, main="Histogram of Number of Doses", xlab="Number of Doses"))
...
THREADS <- 2 # github runners provide 2c 
...
Now we now that 50% of samples have an `NA` value rather than an expression level
...
there is none, we can move on to visualizing our data.
...
```{r transpose_matrix}
t_tpm_matrix <- tpm_matrix |> t() |> as.data.frame()
...
p1 <- ggplot(org_pca) + 
  ...
  theme_classic() + ggtitle("Before Batch Correction")
p1
```
...
adj_tpm_matrix <- ComBat(dat = tpm_matrix, 
...
p2 <- ggplot(adj_pca) + 
  geom_point(aes(x = PC1, y = PC2, color = Batch), size = 5) + 
  scale_color_manual(values = c("#624763", "#B1D3A3")) +
  theme_classic() + ggtitle("After Batch Correction")
